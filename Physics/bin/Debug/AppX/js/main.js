...
function Keyes(plrIndex = 0) {
	for (var i = 0; i < activeKeyes.length; i++) {
		if (activeKeyes[87]) {
			test[plrIndex].Physics(true);
			test[plrIndex].Move(0, -0.1);
		}

		if (activeKeyes[68]) {
			test[plrIndex].Physics(true);
			test[plrIndex].Move(0.1, 0);
		}
...
class GameObject {
	...
	Physics(custom) {
		if (custom || this.OnMouseClick() || this.OnCollide() || this.pos[getY] >= RendHeight() - this.size[getY] * renderer.scale) {
			...
			this.gravityAcceleration = 0;
			this.dir = [this.dir[getX] * this.friction, this.dir[getY] * this.bounce];
		} else {
			this.isGrounded = false;
			this.gravityAcceleration += gravity;
		}

		//this.Move(this.dir[getX] * this.friction, this.dir[getY] * this.friction + this.gravityAcceleration);
		this.Grounded();
	}

	OnMouseClick() {
		if (this.pos[getX] <= mouse.pos[getX]
			&& mouse.pos[getX] <= this.pos[getX] + this.size[getX] * renderer.scale
			&& this.pos[getY] <= mouse.pos[getY]
		...
		for (var i = 0; i < collider.length; i++) {
			if (collider[i] != this) {
				if (this.pos[getX] <= collider[i].pos[getX] + collider[i].size[getX] * renderer.scale
					...
					if (this.oldPos[getX] > collider[i].pos[getY] + collider[i].size[getY] * renderer.scale) {
						this.pos[getX] = this.pos[getX] = collider[i].pos[getY] + collider[i].size[getY] * renderer.scale;
					}
	...
	Grounded() {
		if (!this.isGrounded) {
			this.Move(this.dir[getX], this.dir[getY] + this.gravityAcceleration);
		} else {
	...
	Move(x, y) {
		...
		this.pos[getY] = Math.min(RendHeight() - this.size[getY] * renderer.scale, (this.pos[getY] + y));
			//Math.clamp(this.pos[getY] + y, 0, RendHeight() - this.size[getY] * renderer.scale);
	}

	SetPos(x, y) {
		this.oldPos = [this.pos[getX], this.pos[getY]];
		this.pos[getX] = Math.clamp(x, 0, RendWidth() - this.size[getX] * renderer.scale);
		this.pos[getY] = Math.clamp(y, 0, RendHeight() - this.size[getY] * renderer.scale);
	}
}

var test = [new GameObject(800, 70, 20, 50), new GameObject(10, 10, 20, 20), new GameObject(170, 100, 15, 10)];
test[0].rendObject.color = "blue";
